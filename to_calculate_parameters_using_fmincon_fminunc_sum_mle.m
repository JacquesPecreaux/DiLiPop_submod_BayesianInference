function [ input,fitting_results] = to_calculate_parameters_using_fmincon_fminunc_sum_mle...
    ( input,models,nbEmbryo_givenCondition,simulation,fixed_short_lifetime  )
...
%Options=optimset('TolFun',1e-12,'TolX',1e-12,'MaxFunEvals',5000, 'MaxIter' , 5000);
...
if ~isempty(find(ismember(models,'MonoExpo'),1))
    ...
    [x_prefit, ~, exitflag]=fmincon(@(par) Loglikelihood2_total(par, input,'MonoExpo', @simple_exp2_beta_norm, nbEmbryo_givenCondition, size_population2), ...
        [1], [], [], [], [],[0.1], [10],[],Options);
    [x, ~, exitflag]=fminunc(@(par) Loglikelihood2_total(par, input,'MonoExpo', @simple_exp2_beta_norm, nbEmbryo_givenCondition, size_population2), ...
        x_prefit, Options);
...
if ~isempty(find(ismember(models,'DoubleExpo'),1))
    ...
    [x2_prefit, ~, exitflag]=fmincon(@(par) Loglikelihood2_total(par, input,'DoubleExpo', @double_exp2_beta_norm, nbEmbryo_givenCondition, size_population2), ...
        [0.5 0.5 2], [], [], [], [],[0 0.1 0.1], [1 10 10], [], Options);
    [x2, ~, exitflag]=fminunc(@(par) Loglikelihood2_total(par, input,'DoubleExpo', @double_exp2_beta_norm, nbEmbryo_givenCondition, size_population2), ...
        x2_prefit,Options);    
...
if ~isempty(find(ismember(models,'DoubleExpo_fixedT0'),1)) && exist('fixed_short_lifetime','var') == 1 && ~isempty(fixed_short_lifetime)
    ...
    [x2_prefit_, ~, exitflag]=fmincon(@(par) Loglikelihood2_total(par, input,'DoubleExpo_fixedT0', @double_exp2_beta_norm_fixedT0, ...
        nbEmbryo_givenCondition, size_population2,fixed_short_lifetime), [0.5 2], [], [], [], [],[0 0.1], [1 10], [], Options);
    
    [x2_, ~, exitflag]=fminunc(@(par) Loglikelihood2_total(par, input,'DoubleExpo_fixedT0', @double_exp2_beta_norm_fixedT0, ...
        nbEmbryo_givenCondition, size_population2,fixed_short_lifetime), x2_prefit_,Options);     
...
if ~isempty(find(ismember(models,'DoubleExpo_fixedT0P0'),1)) && exist('fixed_short_lifetime','var') == 1 && ~isempty(fixed_short_lifetime) ...
    ...
    [x2_prefit__, ~, exitflag]=fmincon(@(par) Loglikelihood2_total(par, input,'DoubleExpo_fixedT0P0', @double_exp2_beta_norm_fixedT0P0, ...
        nbEmbryo_givenCondition, size_population2,fixed_short_lifetime,fixed_short_percent), [2], [], [], [], [],[0.1], [10], [], Options);
    
    [x2__, ~, exitflag]=fminunc(@(par) Loglikelihood2_total(par, input,'DoubleExpo_fixedT0P0', @double_exp2_beta_norm_fixedT0P0, ...
        nbEmbryo_givenCondition, size_population2,fixed_short_lifetime,fixed_short_percent), x2_prefit__,Options);     
...
if ~isempty(find(ismember(models,'MonoExpo_stretched'),1))
    ...
    [xS_prefit, ~, exitflag]=fmincon(@(par) Loglikelihood2_total(par, input,'MonoExpo_stretched', @simple_exp2_stretched_beta_norm, nbEmbryo_givenCondition,size_population2), ...
        [1 1],[], [], [], [],[0.1 0], [10 10], [],Options);
    [xS, ~, exitflag]=fminunc(@(par) Loglikelihood2_total(par, input,'MonoExpo_stretched', @simple_exp2_stretched_beta_norm, nbEmbryo_givenCondition,size_population2), ...
        xS_prefit,Options);
...
if ~isempty(find(ismember(models,'TripleExpo'),1))
    ...
    [x3_prefit, ~, exitflag]=fmincon(@(par) Loglikelihood2_total(par, input,'TripleExpo', @triple_exp2_beta_norm, nbEmbryo_givenCondition,size_population2),...
        [0.5 0.5 0.5 2 5 ], [], [], [], [],[0 0.1 0 0.1  0.1], [1 10 1 10 10], [],Options);
    [x3, ~, exitflag]=fminunc(@(par) Loglikelihood2_total(par, input,'TripleExpo', @triple_exp2_beta_norm, nbEmbryo_givenCondition,size_population2),...
        x3_prefit,Options);    
...
if ~isempty(find(ismember(models,'TripleExpo_fixedT0'),1)) && exist('fixed_short_lifetime','var') == 1
    ...
    [x3_prefit_, ~, exitflag]=fmincon(@(par) Loglikelihood2_total(par, input,'TripleExpo_fixedT0', @triple_exp2_beta_norm_fixedT0, nbEmbryo_givenCondition, size_population2, fixed_short_lifetime), ...
         [0.4 0.4 1.5 0.5], [], [], [], [],[0 0 0.1 0.1], [1 1 5 5], [], Options);
    [x3_, ~, exitflag]=fminunc(@(par) Loglikelihood2_total(par, input,'TripleExpo_fixedT0', @triple_exp2_beta_norm_fixedT0, nbEmbryo_givenCondition, size_population2, fixed_short_lifetime), ...
         x3_prefit_,Options);    
