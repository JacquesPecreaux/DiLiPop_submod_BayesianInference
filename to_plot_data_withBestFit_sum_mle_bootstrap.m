function to_plot_data_withBestFit_sum_mle_bootstrap...
...
for iEmbryo = 1 : nbEmbryo_givenCondition
    ...
    if strcmp(best_model,'MonoExpo')
        ...
        string3 = ['residency time = ' num2str(round2(fitting_results.(best_model).T,1e-2)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).T_se_bootstrap,1e-2)) ' s'];
    ...
    elseif strcmp(best_model,'DoubleExpo')
        ...
        string4 = ['residency time = ' num2str(round2(fitting_results.(best_model).T1,1e-2)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).T1_se_bootstrap,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).P1*100,1e-1)) ' \pm ' ...
            num2str(round2(fitting_results.(best_model).P1_se_bootstrap*100,1e-1)) '%'];
        text(25,50,string4,'Units','pixels')
        
        string6 = ['residency time = ' num2str(round2(fitting_results.(best_model).T2,1e-2))  '\pm ' ...
            num2str(round2(fitting_results.(best_model).T2_se_bootstrap,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).P2*100,1e-1)) ' \pm ' ...
            num2str(round2(fitting_results.(best_model).P2_se_bootstrap*100,1e-1)) '%'];
...
%         text(25,100,string1,'Units','pixels')
    ...
    elseif strcmp(best_model,'TripleExpo')
        ...
        string7 = ['residency time = ' num2str(round2(fitting_results.(best_model).TT1,1e-2)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).TT1_se_bootstrap,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).PP1*100,1e-1)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).PP1_se_bootstrap*100,1e-1)) '%'];
        text(25,75,string7,'Units','pixels')
        
        string8 = ['residency time = ' num2str(round2(fitting_results.(best_model).TT2,1e-2)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).TT2_se_bootstrap,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).PP2*100,1e-1)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).PP2_se_bootstrap*100,1e-1)) '%'];
        text(25,50,string8,'Units','pixels')
        
        string9 = ['residency time = ' num2str(round2(fitting_results.(best_model).TT3,1e-2)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).TT3_se_bootstrap,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).PP3*100,1e-1)) '\pm ' ...
            num2str(round2(fitting_results.(best_model).PP3_se_bootstrap*100,1e-1)) '%'];
...
%  
    ...
    if classification_performed == 0
        ...
        namePlot = strcat('tracks_duration_distribution+bestfit-', name0, '-', name_embryo, '-', name1 , '-', name2, '_bootstrap.fig');
    elseif classification_performed == 1
        title(['Duration distributions : ', name1, ' and ', 10, name2, '(', best_model, ' for ', name_embryo, ' )' ]);
        namePlot = strcat('tracks_duration_distribution_T0removal+bestfit-', name0, '-', name_embryo, '-', name1 , '-', name2, '_bootstrap.fig');
    elseif classification_performed == 2
        title(['Duration distributions : ' given_set, ', ', name1, ' and ', 10, name2, '(', best_model, ' for ', name_embryo, ' )' ]);
        namePlot = strcat('tracks_duration_distribution+bestfit_', given_set,'_', name0, '-', name_embryo, '-', name1 , '-', name2, '_bootstrap.fig');
    end
    saveas(gcf,[save_stem namePlot]);
    
%     if classification_performed == 0
%         title(['Duration distributions : ', name1, ' and ', 10, name2, '(', best_model, ' for ', name_embryo, ' )' ]);
%         namePlot = strcat('tracks_duration_distribution+bestfit-', name0, '-', name_embryo, '-', name1 , '-', name2, '_bootstrap.tif');
%     elseif classification_performed == 1
%         title(['Duration distributions : ', name1, ' and ', 10, name2, '(', best_model, ' for ', name_embryo, ' )' ]);
%         namePlot = strcat('tracks_duration_distribution_T0removal+bestfit-', name0, '-', name_embryo, '-', name1 , '-', name2, '_bootstrap.tif');
%     elseif classification_performed == 2
%          title(['Duration distributions : ' given_set, ', ', name1, ' and ', 10, name2, '(', best_model, ' for ', name_embryo, ' )' ]);
%         namePlot = strcat('tracks_duration_distribution+bestfit_', given_set,'_', name0, '-', name_embryo, '-', name1 , '-', name2, '_bootstrap.tif');
%     end
%     saveas(gcf,[save_stem namePlot]);
