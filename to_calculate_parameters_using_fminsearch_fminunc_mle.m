function [ fitting_results ] = to_calculate_parameters_using_fminsearch_fminunc_mle...
    (data,models,timing_phase,blastomere,fitting_results,fixed_short_lifetime,fixed_short_percent )

% fitting of a single embryo using fminsearch function
...
if ~isempty(find(ismember(models,'MonoExpo'),1))
    ...
    [x_prefit, ~, exitflag]=fminsearch(@(par) Loglikelihood2_norm(par, data,'MonoExpo', @simple_exp2_beta_norm, size_population_raw), [1],Options);
    [x, ~, exitflag]=fminunc(@(par) Loglikelihood2_norm(par, data,'MonoExpo', @simple_exp2_beta_norm, size_population_raw), x_prefit,Options);
...
if ~isempty(find(ismember(models,'DoubleExpo'),1))
    ...
    [x2_prefit, ~, exitflag]=fminsearch(@(par) Loglikelihood2_norm(par, data,'DoubleExpo', @double_exp2_beta_norm, size_population_raw), [0.5 0.5 2], Options);
    [x2, ~, exitflag]=fminunc(@(par) Loglikelihood2_norm(par, data,'DoubleExpo', @double_exp2_beta_norm, size_population_raw), x2_prefit, Options);
...
%% fit with double expo with fixed short lifetime as model

if ~isempty(find(ismember(models,'DoubleExpo_fixedT0'),1)) && exist('fixed_short_lifetime','var') == 1 && ~isempty(fixed_short_lifetime)

    [x2_, ~, exitflag]=fmincon(@(par) Loglikelihood2_norm(par, data,'DoubleExpo_fixedT0', ...
        @double_exp2_beta_norm_fixedT0, size_population_raw,fixed_short_lifetime), ...
        [0.5 2], [], [], [], [],[0 0.1], [1 10], [], Options);
    
    fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.parameters = x2_;
    fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.nparam = 2;
    
    if fixed_short_lifetime < 1/x2_(2)
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.P1 = x2_(1);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.T1 = fixed_short_lifetime;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.P2 = 1 - x2_(1);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.T2 = 1/x2_(2);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.flag = exitflag;
        
        %---------------
        % caluclation of R, the denominator in the double_exp2_beta_norm_fixedT0
        % function due to the normalization
        xdata=data(:,1);
        R.double_(1,1) = sum( exp(-xdata./fixed_short_lifetime) );
        R.double_(2,1) = sum( exp(-xdata.*x2_(2)) );
        clear xdata
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.R_normalization = R;
        %----------------
        % calculation fo the size of the population after fitting for each
        % embryo and the total one
        
        size_population.total = double( floor(vpa(sum(double_exp2_beta_norm_fixedT0(x2_,data,R.double_(1,1),R.double_(2,1),...
            size_population_raw,fixed_short_lifetime)),3)) );
        size_population.pop1 = double( x2_(1)*floor(vpa(sum(simple_exp2_beta_norm([1/fixed_short_lifetime],data,R.double_(1,1),size_population_raw)),3)) );
        size_population.pop2 = double( (1-x2_(1))*floor(vpa(sum(simple_exp2_beta_norm([x2_(2)],data,R.double_(2,1),size_population_raw)),3)) );
        percent_population.pop1 = size_population.pop1 / size_population.total *100;
        percent_population.pop2 = size_population.pop2 / size_population.total *100;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.size_population = size_population;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.percentage_population = percent_population;
        
        %-----------------
        % calculate the function value (mi) at each bins and the residues
        % between mi and ci
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.double_ = double_exp2_beta_norm_fixedT0...
            (x2_,data,R.double_(1,1),R.double_(2,1),size_population_raw,fixed_short_lifetime);
        residues = (fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.double_(:) - data(:,2)) ./data(:,2);
        residues(~isfinite(residues))=NaN;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.residuals = cat(2,data(:,1),residues);
        
    elseif 1/x2_(2) < fixed_short_lifetime
        
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.P1 = 1-x2_(1);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.T1 = 1/x2_(2);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.P2 = x2_(1);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.T2 = fixed_short_lifetime;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.flag = exitflag;
        ...
        % function due to the normalization
        xdata=data(:,1);
        R.double_(1,1) = sum( exp(-xdata.*x2_(2)) );
        R.double_(2,1) = sum( exp(-xdata./fixed_short_lifetime) );
        clear xdata
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.R_normalization = R;
        %----------------
        % calculation fo the size of the population after fitting for each
        % embryo and the total one
        size_population.total = double( floor(vpa(sum(double_exp2_beta_norm_fixedT0([(1-x2_(1)) x2_(2) 1/fixed_short_lifetime],data,...
            R.double_(1,1),R.double_(2,1),size_population_raw,fixed_short_lifetime)),3)) );
        size_population.pop1 = double( (1-x2_(1))*floor(vpa(sum(simple_exp2_beta_norm([x2_(2)],data,R.double_(1,1),size_population_raw)),3)) );
        size_population.pop2 = double( x2_(1)*floor(vpa(sum(simple_exp2_beta_norm([1/fixed_short_lifetime],data,R.double_(2,1),size_population_raw)),3)) );
        percent_population.pop1 = size_population.pop1 / size_population.total *100;
        percent_population.pop2 = size_population.pop2 / size_population.total *100;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.size_population = size_population;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.percentage_population = percent_population;
        ...
        % between mi and ci
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.double_ = double_exp2_beta_norm_fixedT0...
            ([(1-x2_(1)) x2_(2) 1/fixed_short_lifetime],data,R.double_(1,1),R.double_(2,1),size_population_raw);
        residues = (fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.double_(:) - data(:,2)) ./data(:,2);
        residues(~isfinite(residues))=NaN;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0.residuals = cat(2,data(:,1),residues);
...
%% fit with double expo with fixed short lifetime and fixed short percent as model
...
if ~isempty(find(ismember(models,'DoubleExpo_fixedT0P0'),1)) && exist('fixed_short_lifetime','var') == 1 && ~isempty(fixed_short_lifetime)...
    ...
    [x2__, ~, exitflag]=fmincon(@(par) Loglikelihood2_norm(par, data,'DoubleExpo_fixedT0P0', ...
        @double_exp2_beta_norm_fixedT0P0, size_population_raw,fixed_short_lifetime,fixed_short_percent), ...
        [0.5], [], [], [], [],[0.1], [10], [], Options);
    
    fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.parameters = x2__;
    fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.nparam = 1;
    
    if fixed_short_lifetime < 1/x2__(1)
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.P1 = fixed_short_percent;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.T1 = fixed_short_lifetime;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.P2 = 1 - fixed_short_percent;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.T2 = 1/x2__(1);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.flag = exitflag;
        
        %---------------
        % caluclation of R, the denominator in the double_exp2_beta_norm_fixedT0P0
        % function due to the normalization
        xdata=data(:,1);
        R.double__(1,1) = sum( exp(-xdata./fixed_short_lifetime) );
        R.double__(2,1) = sum( exp(-xdata.*x2__(1)) );
        clear xdata
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.R_normalization = R;
        %----------------
        % calculation fo the size of the population after fitting for each
        % embryo and the total one
        
        size_population.total = double( floor(vpa(sum(double_exp2_beta_norm_fixedT0P0(x2__,data,R.double__(1,1),R.double__(2,1),...
            size_population_raw,fixed_short_lifetime,fixed_short_percent)),3)) );
        size_population.pop1 = double( fixed_short_percent*floor(vpa(sum(simple_exp2_beta_norm([1/fixed_short_lifetime],data,R.double__(1,1),size_population_raw)),3)) );
        size_population.pop2 = double( (1-fixed_short_percent)*floor(vpa(sum(simple_exp2_beta_norm([x2__(1)],data,R.double__(2,1),size_population_raw)),3)) );
        percent_population.pop1 = size_population.pop1 / size_population.total *100;
        percent_population.pop2 = size_population.pop2 / size_population.total *100;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.size_population = size_population;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.percentage_population = percent_population;
        
        %-----------------
        % calculate the function value (mi) at each bins and the residues
        % between mi and ci
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.double__ = double_exp2_beta_norm_fixedT0P0...
            (x2__,data,R.double__(1,1),R.double__(2,1),size_population_raw,fixed_short_lifetime,fixed_short_percent);
        residues = (fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.double__(:) - data(:,2)) ./data(:,2);
        residues(~isfinite(residues))=NaN;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.residuals = cat(2,data(:,1),residues);
        
    elseif 1/x2__(1) < fixed_short_lifetime
        
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.P1 = 1-fixed_short_percent;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.T1 = 1/x2__(1);
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.P2 = fixed_short_percent;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.T2 = fixed_short_lifetime;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.flag = exitflag;
        ...
        % function due to the normalization
        xdata=data(:,1);
        R.double__(1,1) = sum( exp(-xdata.*x2__(1)) );
        R.double__(2,1) = sum( exp(-xdata./fixed_short_lifetime) );
        clear xdata
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.R_normalization = R;
        %----------------
        % calculation fo the size of the population after fitting for each
        % embryo and the total one
        size_population.total = double( floor(vpa(sum(double_exp2_beta_norm_fixedT0P0([(1-fixed_short_percent) x2__(1) 1/fixed_short_lifetime],data,...
            R.double__(1,1),R.double__(2,1),size_population_raw,fixed_short_lifetime,fixed_short_percent)),3)) );
        size_population.pop1 = double( (1-fixed_short_percent)*floor(vpa(sum(simple_exp2_beta_norm([x2__(1)],data,R.double__(1,1),size_population_raw)),3)) );
        size_population.pop2 = double( fixed_short_percent*floor(vpa(sum(simple_exp2_beta_norm([1/fixed_short_lifetime],data,R.double__(2,1),size_population_raw)),3)) );
        percent_population.pop1 = size_population.pop1 / size_population.total *100;
        percent_population.pop2 = size_population.pop2 / size_population.total *100;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.size_population = size_population;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.percentage_population = percent_population;
        ...
        % calculate the function value (mi) at each bins and the residues
        % between mi and ci
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.double__ = double__exp2_beta_norm_fixedT0P0...
            ([(1-fixed_short_percent) x2__(1) 1/fixed_short_lifetime],data,R.double__(1,1),R.double__(2,1),size_population_raw);
        residues = (fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.double__(:) - data(:,2)) ./data(:,2);
        residues(~isfinite(residues))=NaN;
        fitting_results.(blastomere).(timing_phase).DoubleExpo_fixedT0P0.residuals = cat(2,data(:,1),residues);
...
if ~isempty(find(ismember(models,'MonoExpo_stretched'),1))
    ...
    [xS_prefit, ~, exitflag]=fminsearch(@(par) Loglikelihood2_norm(par, data,'MonoExpo_stretched', @simple_exp2_stretched_beta_norm, size_population_raw), ...
        [0.5 1 1], Options);
    [xS, ~, exitflag]=fminunc(@(par) Loglikelihood2_norm(par, data,'MonoExpo_stretched', @simple_exp2_stretched_beta_norm, size_population_raw), ...
        xS_prefit, Options);
...
if ~isempty(find(ismember(models,'TripleExpo'),1))
    ...
    [x3_prefit, ~, exitflag]=fminsearch(@(par) Loglikelihood2_norm(par, data,'TripleExpo', @triple_exp2_beta_norm, size_population_raw),...
        [0.5 0.5 0.5 2 5 ], Options);
    [x3, ~, exitflag]=fminunc(@(par) Loglikelihood2_norm(par, data,'TripleExpo', @triple_exp2_beta_norm, size_population_raw),...
        x3_prefit, Options);
...
if ~isempty(find(ismember(models,'TripleExpo_fixedT0'),1)) && exist('fixed_short_lifetime','var') == 1
    ...
    [x3_prefit_, ~, exitflag]=fminsearch(@(par) Loglikelihood2_norm(par, data,'TripleExpo_fixedT0', @triple_exp2_beta_norm_fixedT0, size_population_raw, fixed_short_lifetime), ...
        [0.4 0.4 1.5 0.5], Options);
     [x3_, ~, exitflag]=fminunc(@(par) Loglikelihood2_norm(par, data,'TripleExpo_fixedT0', @triple_exp2_beta_norm_fixedT0, size_population_raw, fixed_short_lifetime), ...
        x3_prefit_, Options);
    
    fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0.parameters = x3_;
    fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0.nparam = 4;
...
%% fit with triple expo with fixed short lifetime and percent as model

if ~isempty(find(ismember(models,'TripleExpo_fixedT0P0'),1)) && exist('fixed_short_lifetime','var') == 1  && exist('fixed_short_percent','var') == 1 
    
    [x3_prefit__, ~, exitflag]=fminsearch(@(par) Loglikelihood2_norm(par, data,'TripleExpo_fixedT0P0', @triple_exp2_beta_norm_fixedT0P0, size_population_raw, fixed_short_lifetime), ...
        [0.4 1.5 0.5], Options);
    [x3__, ~, exitflag]=fminunc(@(par) Loglikelihood2_norm(par, data,'TripleExpo_fixedT0P0', @triple_exp2_beta_norm_fixedT0P0, size_population_raw, fixed_short_lifetime), ...
        x3_prefit__, Options);
    
    fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.parameters = x3__;
    fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.nparam = 3;
    
    if 1/x3__(2) < 1/x3__(3)
        
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.P0 = fixed_short_percent;
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.T0 = fixed_short_lifetime;        
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.P1 = x3__(1);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.T1 = 1/x3__(2);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.P2 = 1 - x3__(1) - fixed_short_percent;
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.T2 = 1/x3__(3);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.flag = exitflag;
        
        %---------------
        % caluclation of R, the denominator in the double_exp2_beta_norm
        % function due to the normalization
        xdata=data(:,1);
        R.triple__(1,1) = sum( exp(-xdata.*(1/fixed_short_lifetime)) );
        R.triple__(2,1) = sum( exp(-xdata.*x3__(2)) );
        R.triple__(3,1) = sum( exp(-xdata.*x3__(3)) );
        clear xdata
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.R_normalization = R;
        %----------------
        % calculation fo the size of the population after fitting for each
        % embryo and the total one
        
        size_population.total = double( floor(vpa(sum(triple_exp2_beta_norm_fixedT0(x3__,data,R.triple__(1,1),R.triple__(2,1),R.triple__(3,1),...
            size_population_raw, fixed_short_lifetime)),3)) );
        size_population.pop0 = double( x3__(1)*floor(vpa(sum(simple_exp2_beta_norm([1/fixed_short_lifetime],data,R.triple__(1,1),size_population_raw)),3)) );
        size_population.pop1 = double( x3__(2)*floor(vpa(sum(simple_exp2_beta_norm([x3__(3)],data,R.triple__(2,1),size_population_raw)),3)) );
        size_population.pop2 = double( (1-x3__(1)-x3__(2))*floor(vpa(sum(simple_exp2_beta_norm([x3__(4)],data,R.triple__(3,1),size_population_raw)),3)) );
        percent_population.pop0 = size_population.pop0 / size_population.total *100;
        percent_population.pop1 = size_population.pop1 / size_population.total *100;
        percent_population.pop2 = size_population.pop2 / size_population.total *100;
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.size_population = size_population;
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.percentage_population = percent_population;
        
        %-----------------
        % calculate the function value (mi) at each bins and the residues
        % between mi and ci
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.triple__ = triple_exp2_beta_norm_fixedT0(x3__,data,R.triple__(1,1),R.triple__(2,1),...
            R.triple__(3,1),size_population_raw, fixed_short_lifetime);
        residues = (fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.triple__(:) - data(:,2)) ./data(:,2);
        residues(~isfinite(residues))=NaN;
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.residuals = cat(2,data(:,1),residues);
        
    elseif 1/x3__(3) > 1/x3__(4)
        
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.P0 = x3__(1);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.T0 = fixed_short_lifetime;        
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.P1 = 1 - x3__(1) - x3__(2);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.T1 = 1/x3__(4);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.P2 = x3__(2);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.T2 = 1/x3__(3);
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.flag = exitflag;
        ...
        xdata=data(:,1);
        R.triple__(1,1) = sum( exp(-xdata.*(1/fixed_short_lifetime)) );
        R.triple__(2,1) = sum( exp(-xdata.*x3__(4)) );
        R.triple__(3,1) = sum( exp(-xdata.*x3__(3)) );
        clear xdata
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.R_normalization = R;
        %----------------
        % calculation fo the size of the population after fitting for each
        % embryo and the total one
        size_population.total = double( floor(vpa(sum(triple_exp2_beta_norm_fixedT0([x3__(1) (1-x3__(1)-x3__(2)) x3__(4) x3__(3)],data,...
            R.triple__(1,1),R.triple__(2,1),R.triple__(3,1),size_population_raw, fixed_short_lifetime)),3)) );
        size_population.pop0 = double( x3__(1)*floor(vpa(sum(simple_exp2_beta_norm([1/fixed_short_lifetime],data,R.triple__(1,1),size_population_raw)),3)) );
        size_population.pop1 = double( (1-x3__(1)-x3__(2))*floor(vpa(sum(simple_exp2_beta_norm([x3__(4)],data,R.triple__(2,1),size_population_raw)),3)) );
        size_population.pop2 = double( x3__(2)*floor(vpa(sum(simple_exp2_beta_norm([x3__(3)],data,R.triple__(3,1),size_population_raw)),3)) );
        percent_population.pop0 = size_population.pop0 / size_population.total *100;
        percent_population.pop1 = size_population.pop1 / size_population.total *100;
        percent_population.pop2 = size_population.pop2 / size_population.total *100;
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.size_population = size_population;
        ...
        % calculate the function value (mi) at each bins and the residues
        % between mi and ci
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.triple__ = triple_exp2_beta_norm_fixedT0([(x3__(1)) (1-x3__(1)-x3__(2)) x3__(4)  x3__(3)],...
            data,R.triple__(1,1),R.triple__(2,1),R.triple__(3,1),size_population_raw, fixed_short_lifetime);
        residues = (fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.triple__(:) - data(:,2)) ./data(:,2);
        residues(~isfinite(residues))=NaN;
        fitting_results.(blastomere).(timing_phase).TripleExpo_fixedT0P0.residuals = cat(2,data(:,1),residues);
...
if ~isempty(find(ismember(models,'QuadroExpo'),1))
    ...
    [x4_prefit, ~, exitflag]=fminsearch(@(par) Loglikelihood2_norm(par, data,'QuadroExpo', @quadro_exp2_beta_norm, size_population_raw), ...
        [0.25 0.3 0.25 0.6 0.25 1 2], Options);
    [x4, ~, exitflag]=fminunc(@(par) Loglikelihood2_norm(par, data,'QuadroExpo', @quadro_exp2_beta_norm, size_population_raw), ...
        x4_prefit, Options);
