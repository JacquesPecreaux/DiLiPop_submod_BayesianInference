function to_plot_data_withBestFit_sum_mle_noError...
...
for iEmbryo = 1 : nbEmbryo_givenCondition
    ...
    if strcmp(best_model,'MonoExpo')
        ...
        string3 = ['residency time = ' num2str(round2(fitting_results.(best_model).T,1e-2)) ' s'];
    ...
    elseif strcmp(best_model,'DoubleExpo')
        ...
        string6 = ['residency time = ' num2str(round2(fitting_results.(best_model).T2,1e-2))  ' s and ' ...
    ...
    elseif strcmp(best_model,'MonoExpo_stretched')
        ...
        string3 = ['residency time = ' num2str(round2(fitting_results.(best_model).Ts,1e-2)) ' sec. and power = '...
            num2str(round2(fitting_results.(best_model).power,1e-2))];
        text(25,50,string3,'Units','pixels')
    ...
    elseif strcmp(best_model,'TripleExpo')
        ...
        string7 = ['residency time = ' num2str(round2(fitting_results.(best_model).TT1,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).PP1*100,1e-1)) ' % '];
        text(25,75,string7,'Units','pixels')
        
        string8 = ['residency time = ' num2str(round2(fitting_results.(best_model).TT2,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).PP2*100,1e-1)) ' % '];
        text(25,50,string8,'Units','pixels')
        
        string9 = ['residency time = ' num2str(round2(fitting_results.(best_model).TT3,1e-2)) ' s and ' ...
    ...
    elseif strcmp(best_model,'TripleExpo_fixedT0')
        ...
        string7 = ['residency time = ' num2str(round2(fitting_results.(best_model).T0,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).P0*100,1e-1)) ' % '];
        text(25,75,string7,'Units','pixels')
        
        string8 = ['residency time = ' num2str(round2(fitting_results.(best_model).T1,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).P1*100,1e-1)) ' % '];
        text(25,50,string8,'Units','pixels')
        
        string9 = ['residency time = ' num2str(round2(fitting_results.(best_model).T2,1e-2)) ' s and ' ...
            num2str(round2(fitting_results.(best_model).P2*100,1e-1)) ' % '];
        text(25,25,string9,'Units','pixels')
    ...
    elseif strcmp(best_model,'QuadroExpo')
        ...
        %-------------
    % best model = Diffusion-drift model
    
    elseif strcmp(best_model,'Drift_diffusion')
              
        f_DD = @(x) size_population.(name_embryo).raw / fitting_results.(best_model).R_normalization.(name_embryo).dd * x.^(-3/2) ...
            .* exp(-x/fitting_results.(best_model).T);
        
        fplot(f_DD,[0 max(binranges)],'-r');
